...
| ``build:server``| 桌面版(SEA)用にサーバーを webpack バンドルする。

## ネット対戦
同梱のネット対戦サーバーで LAN 内やインターネット経由の対戦ができる。

### Web 版 (npm run server)
```sh
npm run build     # dist/ を作成(初回のみ)
npm run server    # http://localhost:3830/ で待ち受け
```
ブラウザで `http://<サーバー>:3830/` を開き、タイトル画面の「ネット対戦」から入室。
空席は AI が補う。詳細は [server/README.md](server/README.md)。

### 桌面版(SEA 单文件)
内置完整对战服务器，双击即可在局域网内一键联机：
```sh
cd desktop-sea
npm run build     # 産出 `電脳麻将`(Linux) / `電脳麻将.exe`(Windows)
```
双击 `電脳麻将` → 浏览器自动打开 → タイトル画面の「ネット対戦」→
「局域网联机」按钮开启 → 邀请链接发给其他设备即可加入对局。
默认仅监听 `127.0.0.1`（仅本机访问），不会影响局域网。
...
- 思考ルーチン
  - [打牌選択アルゴリズム(10) 〜 残り牌数の正規化](https://blog.kobalab.net/entry/2025/07/26/204059) (v2.5)
  - [押し引きアルゴリズムの改善(4) 〜 押し引き閾値の再調整](https://blog.kobalab.net/entry/2025/07/27/221043) (v2.5)
  - [牌の危険度計算アルゴリズム(5) 〜 危険度閾値の再調整](https://blog.kobalab.net/entry/2025/07/29/074139) (v2.5)
- 表示処理
  ...
  - [電脳麻将UI 〜 flipInput](https://blog.kobalab.net/entry/2026/03/17/011103) (v2.5)
  - [電脳麻将UI 〜 牌](https://blog.kobalab.net/entry/2026/03/19/071224) (v2.5)
  - [電脳麻将UI 〜 音声](https://blog.kobalab.net/entry/2026/03/21/140015) (v2.5)
  ...
  - [電脳麻将UI 〜 layout](https://blog.kobalab.net/entry/2026/04/01/200641) (v2.6)
  - [電脳麻将UI 〜 盤面](https://blog.kobalab.net/entry/2026/04/06/083417) (v2.5)
  - [電脳麻将UI 〜 設計思想](https://blog.kobalab.net/entry/2026/04/09/081902) (v2.5)
  - [電脳麻将UI 〜 対局者(基盤)](https://blog.kobalab.net/entry/2026/04/13/001330) (v2.5)
  - [電脳麻将UI 〜 対局者(対AI戦)](https://blog.kobalab.net/entry/2026/04/18/122703) (v2.5)
  - [電脳麻将UI 〜 対局者(ネット対戦)](https://blog.kobalab.net/entry/2026/04/23/074307) (v2.5)
  - [電脳麻将UI 〜 VoiceOver対応(0)](https://blog.kobalab.net/entry/2026/04/28/072506) (v2.5)
- 牌譜エディタ
