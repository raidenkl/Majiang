### v2.5.5 / 2026-09-21
  ...
  - ネット対戦を自前実装（express + socket.io + Majiang.Game 权威引擎）
    - ルーム作成/入室、CPU 補充、切断時の自動代打と再接続復帰
    - 通知メッセージ（開局/和了/流局/終局）を「クリックで進む」に変更
  - 和了演出をネット対戦にも適用（役を一つずつ表示＋読み上げ、ドラ別計上、満貫以上の段位音声）
  - Chromium の WebMediaPlayer 上限による音声欠落を修正（preload=none + 再生後の解放）
  - デスクトップ版(SEA)に完全な対戦サーバーを内蔵、ワンクリックで LAN 対戦
  - 端末のニックネーム変更に対応、認証プローブによる上書きを修正

### v2.5.3 / 2026-08-22

  - ネット対戦の牌譜保存のタイミングを変更
  - キーボードが使えるタブレットでは牌譜エディタが使えるよう修正
    - @kobalab/majiang-ui v1.6.0 → v1.6.1
  - @kobalab/majiang-ai 1.1.0 → 1.2.0

### v2.5.2 / 2026-08-16

  - ネット対戦機能で「既に接続済みです」のエラーが出る問題に対処
  - OAuth認証のロゴの表示を待たないよう修正
  - 脆弱性警告に対処
...
### v2.5.1 / 2026-03-08
  ...
  - iPhoneのChromeでアドレスバーに盤面が隠れてしまう問題に対処
  - パッケージを最新化
    - @kobalab/majiang-core 1.3.4 → 1.3.5
  - 脆弱性警告に対処
    - ajv 6.12.6 → 8.18.0
    - glob 10.4.5 → 10.5.0
...
## v2.5.0 / 2025-07-29
  ...
  - @kobalab/majiang-ui 1.5.0 → 1.6.0
    - 牌譜ビューアに天鳳のJSON形式の牌譜をエクスポートする機能を追加
    - 牌譜ビューアの検討モードがリーチ選択を表示するよう修正
    - 牌譜エディタの和了情報入力画面で裏ドラを変更できないバグを修正
  - 何切る解答機に「巡目」を追加
  - 何切る解答機に「立体何切る」の機能を追加
  - 牌画入力ツールの画像と加槓の入力方法を変更
  - @kobalab/majiang-core 1.3.2 → 1.3.4
    - 14枚形以外の入力を打牌可能な状態にしないよう修正

### v2.4.17 / 2025-06-17

  - 牌譜ビューアに対局者名を仮名表示にする機能を追加
    - @kobalab/majiang-ui v1.4.15 → v1.5.0
  - 脆弱性警告に対処
    - brace-expansion 2.0.1 → 2.0.2

### v2.4.16 / 2025-03-24

  - @kobalab/majiang-ui 1.4.14 → 1.4.15
